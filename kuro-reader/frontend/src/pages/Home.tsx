...
/**
 * Cleans chapter number from prefixes
 */
const cleanChapterNumber: any = (chapter: string | undefined): string => {
  if (!chapter) return '';
  return chapter
...
function Home() {
  ...
  const loadAllContent = useCallback(async () => {
    ...
    try {
      ...
      const processResult: any = (result, fallback = []) => {
        if (result.status === 'fulfilled') {
          const data = Array.isArray(result.value) 
            ? result.value 
            : result.value?.results || [];
          
          return data.slice(0, 6).map((item: any) => ({ // RIDOTTO: 8 → 6 per LCP migliore
            ...item,
