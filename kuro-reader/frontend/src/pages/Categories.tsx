...
/**
 * CATEGORIES - Browse manga by categories and genres
...
function Categories(): JSX.Element {
  ...
  const [searchResults, setSearchResults] = useState<Manga[]>([]); // Risultati ricerca dalla navbar
  ...
  const loadPresetData = async (preset: string, pageNum: number, reset = false): Promise<void> => {
    ...
    try {
      ...
      if (result && result.results) {
        const deduped = Array.from(new Map(result.results.map((item: any) => [item.url, item])).values());
        if (reset) {
          ...
          setMangaList(prev => {
            ...
            const newItems = deduped.filter((m: Manga) => !existingUrls.has(m.url));
  ...
  const loadMoreSearch = async (pageNum: number): Promise<void> => {
    ...
    try {
      ...
      setMangaList(prev => {
        const existingUrls = new Set(prev.map(m => m.url));
        const newItems = results.filter((m: Manga) => !existingUrls.has(m.url));
        return [...prev, ...newItems];
      });
      setHasMore(more);
  ...
  const toggleGenre: any = (genreId: string): void => {
  ...
  const runSearch = async ({ page = 1 }) => {
    ...
    const intersect = lists.flat().filter((item: any) => countByUrl.get(item.url) === selectedGenres.length);
  ...
  const searchWithFilters = async () => {
    ...
    try {
      ...
      const deduped = Array.from(new Map(results.map((item: any) => [item.url, item])).values());
      setMangaList(deduped);
      setHasMore(hasMore);
      setTotalLoaded(deduped.length);
    } catch (error: unknown) {
